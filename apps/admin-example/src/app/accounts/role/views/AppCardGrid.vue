...
import type { AppCardGridProps, AppCardGridSlots } from "@/pkg/types";
...
} = defineProps<AppCardGridProps<TRecord>>();
const slots = defineSlots<AppCardGridSlots>();
const { t } = useI18n();
...
const infoClass = layout == "vertical" ? "" : "relative col-span-2";
</script>
<template>
  <article
  ...
  >
    ...
    <!-- Info Section -->
    <section class="flex flex-col h-full" :class="`${pt.info} ${infoClass}`">
      <slot name="card-info">
        <div class="info-top px-4 pt-4">
          <AppBtn
            ...
            "
            justify="start"
            class="font-bold mb-2 max-w-[80%] text-2xl"
            :class="pt.title || ''"
          >
            <template #label>
      ...
      <slot name="footer">
        ...
        >
          ...
          <slot name="footer-start">
            <slot name="createdAt" :class="pt['footer-start'] || ''">
              <time
            ...
            <slot name="deletedAt" :class="pt['footer-start'] || ''">
              <time
