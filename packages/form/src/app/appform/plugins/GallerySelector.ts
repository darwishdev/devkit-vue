...
import { UIPlugin, UIPluginOptions } from '@uppy/core'
...
export default class GallerySelectPlugin extends UIPlugin<GalleryPluginOptions, any, any, GalleryPluginState> {
  private shouldRenderGallery = false;
  private icon: () => h.JSX.Element
  private selectedImages = new Set<string>()
  constructor(uppy: any, opts: GalleryPluginOptions) {
    super(uppy, opts)
  ...
  private async handleSubmitGallery() {
    ...
    for (const url of selected) {
      ...
      try {
        ...
        this.uppy.emit('upload-success', this.uppy.getFile(fileID), { status: 200 })
        console.log('holaahe')
        console.log(this.uppy.getFiles())
      } catch (err) {

        console.log('holaahe', err)
        this.uppy.log(`[${this.id}] error adding ${fullSrc}: ${(err as Error).message}`, 'error')
      }
    }
  ...
  render() {
    ...
    return h(
      'div',
      { style: 'padding: 1rem; color: #333;' },
      [
        h('div', { style: 'margin-bottom: 0.5rem; font-weight: bold;' }, 'Select images to add:'),

        // Grid of thumbnails
        h(
          'div',
          { style: 'display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 1rem;' },
          images.map((url) => {
